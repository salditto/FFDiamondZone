...
import { faHome, faInfoCircle, faShoppingCart, faQuestionCircle, faEnvelope, faBars, faTimes, faGlobe } from '@fortawesome/free-solid-svg-icons';

const Navbar = ({ onFAQClick, onHomeClick, onContactClick, onBuyClick, onAboutClick }) => {
  const { t, i18n } = useTranslation();
  const [scrolled, setScrolled] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [currentLanguage, setCurrentLanguage] = useState(i18n.language.startsWith('es') ? 'Español' : 'English');
  ...
  const navLinks = [
    ...
    { label: t('navbar.about'), icon: faInfoCircle, handler: onAboutClick, href: '#about-section' },
    { label: t('navbar.buy'), icon: faShoppingCart, handler: onBuyClick, href: '#buy-diamonds' },
    { label: t('navbar.faq'), icon: faQuestionCircle, handler: onFAQClick, href: '#faq' },
    { label: t('navbar.contact'), icon: faEnvelope, handler: onContactClick, href: '#contact' },
  ];
    ...
    <nav className={`navbar ${scrolled ? 'scrolled' : ''} ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
      ...
      <div className="navbar-container">
        ...
        <div className="language-selector desktop-lang">
          <button className="lang-trigger" onClick={() => setIsLangDropdownOpen(!isLangDropdownOpen)} aria-haspopup="true" aria-expanded={isLangDropdownOpen}>
            <FontAwesomeIcon icon={faGlobe} />
          </button>
          {isLangDropdownOpen && (
            <div className="lang-dropdown">
              <div className="dropdown-arrow"></div>
              <button onClick={() => changeLanguage('es', 'Español')} className={currentLanguage === 'Español' ? 'active' : ''}>Español</button>
              <button onClick={() => changeLanguage('en', 'English')} className={currentLanguage === 'English' ? 'active' : ''}>English</button>
            </div>
          )}
        </div>
